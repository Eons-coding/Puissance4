...
public class Grille extends JFrame implements MouseListener, ActionListener, WindowListener {
	...
	public Grille(int nbRow, int nbCol, Jeu jeu) {
		...
		nbGrilles++;   // Une nouvelle grille a �t� cr��e
	...
	public void actionPerformed(ActionEvent actionEvent) {
		...
		if (src == undo) {
			Case c = (Case)this.pane.getComponent(0); // On prend par exemple la 1�re case pour r�cup�rer le Jeu
			c.jeu.undo();
		}
		else if (src == saveas) {
		...
		/*else if (src == fermer) {
			int ok = Saisie.question_ouinon("Etes-vous s�r de vouloir fermer toutes les fen�tres ?", "Fermer le programme");
			if (ok == 0)
				System.exit(0);
		}*/
		else if (src == comput) {
